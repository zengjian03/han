...
let imghost = 'https://img.jgsfnl.com';
...
async function init(cfg) {
    ...
    try {
        let res = await req(`${host}/api/v2/settings/resourceDomainConfig`, { headers: { 'User-Agent': UA } });
        let config = JSON.parse(res.content);
        if (config.code === 1 && config.data && config.data.imgDomain) {
            imghost = randomImageDomain(config.data.imgDomain);
        }
    } catch (e) {
        console.error('获取图片域名失败:', e.message);
    }
}

function randomImageDomain(domains) {
