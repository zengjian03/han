# 本地资源管理.py - 智能多策略封面提取版
# 说明：MP3/FLAC/M4A/AAC/OGG/WAV 多策略提取内置封面
# 优化：元数据解析 + 原始图片搜索 + 智能回退，保证最高成功率
...
print("ℹ️ 本地资源管理加载成功 - 智能多策略封面提取版（预加载500首）")
...
class Spider(Spider):
    ...
    def _add_audio_info_fast(self, result, file_path):
        ...
        if not result.get("poster"):
            result["poster"] = self.DEFAULT_AUDIO_ICON
    ...
    def _merge_split_urls(self, content):
        ...
        while i < len(lines):
            ...
            # 如果当前行是纯数字开头（如 "1"、"2"等），单独处理
            if re.match(r'^\d+\s*$', line):
                merged_lines.append(line)
                i += 1
                continue
            
            # 如果当前行以 http:// 或 https:// 开头，开始收集URL
            if line.startswith(('http://', 'https://')):
                current_url = line
                i += 1
                # 继续收集后续行直到遇到完整的URL或下一行不是URL片段
                while i < len(lines):
                    next_line = lines[i].strip()
                    # 如果下一行是数字、空行或者新的http开头，停止收集
