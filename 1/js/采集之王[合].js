...
`.trim(),
            ...
            for (const it of json) {
                ...
                try {
                    ...
                    if (it.class_name && it.class_url) {
                        ...
                        json1 = JSON.parse(await request(urljoin(_obj.type_id, _obj.api || rule.classUrl))).class;
                    }
    ...
    推荐: async function () {
        ...
        let update_info = [{
            ...
            vod_pic: urljoin(publicUrl, './images/logo.png'),
        }];
        let VODS = [];
        if (rule.classes) {
            ...
            try {
                let html = await request(_url, {timeout: rule.timeout});
                let json = JSON.parse(html);
                VODS = json.list;
                VODS.forEach(it => {
                ...
                if (start < canSearch.length) {
                    ...
                    if (typeof (batchFetch) === 'function') {
                        let reqUrls = urls.map(it => {
                            return {
                                url: it.split('#')[0], // 去掉 #page=页码
                                options: {timeout: rule.timeout}
                            }
                        });
                        let rets = await batchFetch(reqUrls);
                        let detailUrls = [];
                        ...
                        let rets2 = reqUrls2.length > 0 ? await batchFetch(reqUrls2) : [];
                            ...
                            try {
                                ...
                                if (data.length > 0) {
                                    if (rule.search_pic && !data[0].vod_pic) {
                                        ...
                                        let detailUrl = urls[idx].split('wd=')[0] + 'ac=detail&ids=' + data.map(k => k.vod_id.split('$')[1]).join(',');
                                        try {
                                            let detailJson = JSON.parse(await request(detailUrl));
                                            log('二级数据列表元素数:' + detailJson.list.length);
                                            data.forEach((d, _seq) => {
                                                // let detailVodPic = detailJson.list[_seq].vod_pic;
                                                let detailVodPic = detailJson.list.find(vod => vod.vod_id.toString() === d.vod_id.split('$')[1]);
...
function getRandomItem(items) {//从列表随机取出一个元素
